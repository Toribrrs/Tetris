...
class Tetris:
    def __init__(self, window_size):
        ...
        self.has_swapped = False  # Para evitar múltiplas trocas por peça
    ...
    def draw_ghost_piece(self):
        ...
        while True:
            ...
            for y in range(len(self.shape_matrix)):
                for x in range(len(self.shape_matrix[0])):
                    if self.shape_matrix[y][x] == 1:
                        try:
                            if self.map[ghost_pos[1] + y][ghost_pos[0] + x] != '':
                                collided = True
                                break
                        except:
                            collided = True
        ...
        # Desenha a silhueta
        ghost_color = tuple(min(c + 100, 255) for c in self.shape[self.selected_form]['color'])
        for y in range(len(self.shape_matrix)):
            for x in range(len(self.shape_matrix[0])):
                if self.shape_matrix[y][x] == 1:
                    px = self.board_square * (x + ghost_pos[0])
                    py = self.board_square * (y + ghost_pos[1])
                    pg.draw.rect(self.window, ghost_color, (px, py, self.board_square, self.board_square), 1)
